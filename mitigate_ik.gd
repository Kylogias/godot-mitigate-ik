...
func set_active(val):
	active = val
	if active and not (get_parent() is Skeleton):
		push_warning("(from MitigateIK3D) WARNING: The script may not work properly if this node isn't a child of the model")
	set_process(val)
